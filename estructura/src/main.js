...
function renderHistory(query = '') { const term = query.trim().toLowerCase(); const records = state.solicitudes.filter((s) => [s.empresa, s.sector, s.estado, s.cedulaJuridica].some((value) => String(value || '').toLowerCase().includes(term))); $('#history-count').textContent = `${records.length} registro(s)`; $('#history-table').innerHTML = `<table><thead><tr><th>Fecha de registro</th><th>Empresa</th><th>Sector</th><th>Inversión</th><th>Resultado IA</th><th>Acción</th></tr></thead><tbody>${records.map((s) => `<tr><td>${escapeHtml(s.creadoEn || '—')}</td><td><strong>${escapeHtml(s.empresa)}</strong><small>${escapeHtml(s.cedulaJuridica)}</small></td><td>${escapeHtml(s.sector)}</td><td>${money(s.inversionProyectada)}</td><td>${s.puntaje ?? 'Pendiente'} ${badge(s.estado)}</td><td><button class="table-action detail-trigger" data-id="${s.id}">Ver detalle</button></td></tr>`).join('') || '<tr><td colspan="6" class="empty">No hay coincidencias en el historial.</td></tr>'}</tbody></table>`; }
